...
class MLPHead(nn.Module):
    ...
    def __init__(self, emb_dim, num_classes, dropout=0.1):
        ...
        self.fc1 = nn.Linear(emb_dim, emb_dim // 2) # fc layer 
        self.gelu = nn.GELU() # GELU activation
        self.fc2 = nn.Linear(emb_dim // 2, num_classes) # fc layer 
...
def test(model, dataloader, criterion, resnet_features=None):
...
if __name__ == '__main__':
    ...
    # init model 
    model = ViT(patch_size, max_len, emb_dim, num_classes, num_layers, num_channels, heads).to(device)
    
    resnet_features = None
    
    # training and testing dataloader
    train_dataloader = CIFARDataLoader(train= True, batch_size=batch_size, num_workers=num_workers, shuffle=True, size='32')
    test_dataloader = CIFARDataLoader(train= False, batch_size=batch_size, num_workers=num_workers, shuffle=False, size='32')
